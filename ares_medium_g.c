...
int main(int argc, char **argv)
{
    ...
    int file_exist = exists("ares_medium_g.csv");
    ...
    if (rank == 0)
    {
        if (file_exist == 0)
        {
            file = fopen("ares_medium_g.csv", "w");
            // Write the header row to the output file
            fprintf(file, "Cores, Time\n");
        }
        else
        {
            file = fopen("ares_medium_g.csv", "a");
        }
        if (file == NULL)
        {
    ...
    for (z = 0; z < experiment_iterations; z++)
    {
    ...
    if (rank == 0)
    {
        // fprintf(file, "======= END OF Measurement =====");
        fclose(file);
    }
